...
namespace CivilFX.TrafficV5
{
    ...
    public class SignalPath
    {
        ...
        public void ActivateStopSign(int crossingPathVehicleNum)
        {
            if(crossingPathVehicleNum == 0)
            {
                pathController.RemoveObstacles(new List<VehicleController>(obstacles));
    ...
    public class IntersectionController : MonoBehaviour
    {
        ...
        public TrafficPathController pathToCross;
        public int numLanesToCross;
        ...
        private void Update()
        {
            ...
            foreach (var signalPath in signalPaths)
            {
                ...
                // For "stop signs" instead of signal lights

                if (isStopSign)
                {
                    int otherPathVehiclesCount = 0;

                    for (int i = 0; i < numLanesToCross; i++)
                    {
                        var otherPathVehiclesPerLane = pathToCross.GetTargetNeighbourhood(0, pathToCross.path.pathLength, i);

                        otherPathVehiclesCount += otherPathVehiclesPerLane.Count;
                    }

                    signalPath.ActivateStopSign(otherPathVehiclesCount);
                }
