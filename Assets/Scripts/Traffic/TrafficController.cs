...
namespace CivilFX.TrafficV5
{
    ...
    public class TrafficController : MonoBehaviour
    {
        ...
        [Header("Inflow Visualization")]
        public GameObject objTemplate;
        public Color selectedColor = Utilities.CustomColor.civilGreen;
        public Color deselectedColor = Color.magenta;
        private SpawnPointGroupController spawnPointGroupController;


        [Header("Diverge Visualization")]
        public GameObject divergeObjTemplate;
        public Color divergeSelectedColor = Utilities.CustomColor.civilGreen;
        public Color divergeDeselectedColor = Color.cyan;
        private DivergePointGroupController divergePointGroupController;
        ...
        void Awake()
        {
            ...
            //spawn and diverge points

            spawnPointGroupController = new SpawnPointGroupController(selectedColor, deselectedColor);
            divergePointGroupController = new DivergePointGroupController(divergeSelectedColor, divergeDeselectedColor);

            foreach (var pathController in allPathControllers)
            {
                if (pathController.allowRespawning)
                {
                    var pos = pathController.GetRawPosition(0) + new Vector3(0, 5, 0);
                    var spawnPoint = Instantiate(objTemplate, pos, Quaternion.identity, gameObject.transform);
                    spawnPoint.name = $"SP: {pathController.name}";
                    var script = spawnPoint.GetComponent<SpawnPointController>();
                    script.pathController = pathController;
                    spawnPointGroupController.Add(script);
                }

                foreach(var ramp in pathController.ramps)
                {
                    if(ramp.type == Ramp.RampType.Diverged)
                    {
                        var divergePos = ramp.newPath.GetRawPosition(0) + new Vector3(0, 5, 0);
                        var divergePoint = Instantiate(divergeObjTemplate, divergePos, Quaternion.identity, gameObject.transform);
                        divergePoint.name = $"SP: {ramp.newPath.name}";
                        var divergeScript = divergePoint.GetComponent<DivergePointController>();
                        divergeScript.pathEntering = ramp.newPath;
                        divergeScript.pathDiverging = pathController;
                        divergePointGroupController.Add(divergeScript);
                    }
                }
            }

            spawnPointGroupController.HideAllSpawnPoints();
            divergePointGroupController.HideAllDivergePoints();
        }
        ...
        public void SetRuntimeVisualInflowCount(bool enable)
        {
            ...
            if (enable)
            {
                spawnPointGroupController.ShowAllSpawnPoints();
            }
            else
            {
                spawnPointGroupController.HideAllSpawnPoints();
        ...
        public void SetRuntimeVisualDivergeCount(bool enable)
        {
            if (!gameObject.activeInHierarchy)
            {
                return;
            ...
            if (enable)
            {
                divergePointGroupController.ShowAllDivergePoints();
            }
            else
            {
                divergePointGroupController.HideAllDivergePoints();
        ...
        public void ToggleTraffic(bool trafficOn)
        {
            foreach (var pathController in allPathControllers)
            {
                pathController.ToggleTraffic(waitingVehicles, trafficOn);
                pathController.allowRespawning = trafficOn;
            }
        }
        ...
        public void ResetSimulation()
        {
            foreach (var pathController in allPathControllers)
            {
                pathController.ResetSimulation(waitingVehicles);
            }
        }
    }
}