...
namespace CivilFX.TrafficV5
{
    ...
    public class TrafficPathController : MonoBehaviour
    {
        ...
        public void Init(GameObject[] _prefabs, int _vehiclesCount, List<VehicleController> waitingVehicles)
        {
            ...
            ////Special cases for diverge percents
            //if (gameObject.name == "Fairgrounds SB (2)")
            //    {
            //        UpdateVehicleDivergePercentage(100);
            //    }

            //    // Default 0
            //    else
                    UpdateVehicleDivergePercentage(0);
        }
        ...
        public void UpdateVehicleDivergePercentage(int newDivergePercent)
        {
            vehicleDivergePercentage = newDivergePercent;
            vehicleDivergeAllowed = Random.Range(1, 100) <= vehicleDivergePercentage;
            Debug.Log(vehicleDivergePercentage);
        }


        public void ToggleTraffic(List<VehicleController> waitingVehicles, bool trafficOn)
        {
            List<VehicleController> tempVehicles = new List<VehicleController>(10);
        ...
        public void ResetSimulation(List<VehicleController> waitingVehicles)
        {
            List<VehicleController> tempVehicles = new List<VehicleController>(10);
        ...
        public Vector3 GetRawPosition(float u)
        {
        ...
        public void MergeDiverge(Ramp ramp)
        {
            ...
            foreach (var vehicle in vehicles)
            {
                if (!vehicle.IsVirtual)
                {
                    ...
                    else if (rampType == Ramp.RampType.Diverged 
                              //&& vehicleDivergeAllowed
                             && vehicle.allowLeavingPath && vehicle.curvePos >= uEnd && vehicle.curvePos <= uDiverge
                             )
                    {
                        changingVeh = vehicle;
                    }
